...
export default function AuthPage() {
  ...
  const subtitle = { login: 'Welcome back', register: 'Start planning your trips', forgot: 'Reset your password' }
    ...
    <div className="h-full overflow-y-auto scrollbar-hide bg-bg page-enter flex flex-col">
      <div className="flex-1 flex flex-col justify-center px-6 py-8 max-w-sm mx-auto w-full">
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-text-3 text-sm mb-8 self-start"
        >
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-text-3 text-sm mt-1">{subtitle[mode]}</p>
        </div>
        ...
        {forgotSent ? (
          <div className="text-center space-y-4">
            ...
            <p className="text-text-1 font-semibold">Check your inbox</p>
            <p className="text-text-3 text-sm">If that email is registered, we've sent a reset link. It expires in 1 hour.</p>
            <button onClick={() => switchMode('login')} className="btn-primary mt-2">
              Back to sign in
            </button>
          </div>
        ) : (
            ...
            {mode === 'login' && (
              <div className="text-right">
                <button type="button" onClick={() => switchMode('forgot')}
                  className="text-text-3 text-xs hover:text-gold transition-colors">
                  Forgot password?
                </button>
              </div>
            )}
